...
| Status says `baseline-unknown` after manual setup | The OS window existed before the watcher loaded, or an external writer’s exact image is unknown. | Follow the [startup requirement](./installation.md#startup-and-existing-windows). Resume cannot reconstruct unknown image state. |
